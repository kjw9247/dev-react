...
const EmpList = () => {
  ...
  //클라우디너리 이용할때 const [file2, setFile2] = useState({fileNmae:null, fileURL: null})  
  ...
  // 아래 세 줄은 입력모달과 관련된 코드이다
  const [show, setShow] = useState(false)
  const handleClose = () => setShow(false)
  const handleShow = () => setShow(true)
  // 아래 8개의 함수는 사용자가 입력한 값이 변하면
  // 그때 마다 상태값 갱신한다
  // 컴포넌트가 렌더링 될때마다. 아래 함수가 매번 8개씩 새로 복제된다
  ...
  // 의존성 배열을 생략하면 EmpList렌더링이 될때 마다 호출됨
  // 의존성 배열이 비어 있으면 최초 단 1번만 호출됨
  // 의존성 배열에 변수(useState)가 오면 그 값이 변경될때마다 호출됨
  // EmpList가 렌더링이 될 때 -> useEffect()
  useEffect(()=>{
    const asyncDB = async() => {
      const emp = { gubun: null, keword: null }
      const res = await empListDB(emp)
      // axios일때 data를 반드시 붙여야한다 한다
